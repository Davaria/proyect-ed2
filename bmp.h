...
class BitMap {
    ...
    public:
        ...
        void encrypt(char* _file,int k);
        void decrypt(char* _file, int k);
};
...
std::vector<unsigned int> BitMap::getPixel(int x, int y){
    if(x<m_width && y<m_height){
        ...
        y = m_height -1- y; //to flip thingsm_pixelData[cont];
        //std::cout<<"y: "<<y<<" x: "<<x<<"\n";
        ...
        std::cout<<"Pixel array"<<" R "<<m_rowSize*y+3*x+2<<" G "<<m_rowSize*y+3*x+1<<" B "<<m_rowSize*y+3*x+0<<std::endl;
        std::cout<<"R "<<v[0]<<" G "<<v[1]<<" B "<<v[2]<<std::endl;
...
void BitMap::writePixel(int x,int y, int R, int G, int B) {
    if (x >= m_width || y >= m_height){
        std::cout<<"Error en el indice"<<std::endl;
    }
...
void BitMap::encrypt(char* _file, int k){
    //std::ofstream out(_file);

    unsigned char matriz[m_height][m_width*3];
    std::cout<<m_pixelArraySize<<std::endl;
    std::cout<<m_rowSize<<std::endl;

    int temp=0,cont=0,n=0;
    for (int i = 0; i < m_height; i++){
        for (int j = 0; j < m_width*3; j=j+3){
            temp = m_height - 1 - i;
            matriz[i][j+2] = m_pixelData[m_rowSize*temp+j+2];
            matriz[i][j+1] = m_pixelData[m_rowSize*temp+j+1];
            matriz[i][j] = m_pixelData[m_rowSize*temp+j];
        }
    }
    ...
    for (int i = 0; i < m_height; i++){
        for (int j = 0; j < m_width*3; j=j+3){
            n=(cont*k)%m_width;
            writePixel(n,i,matriz[i][j+2],matriz[i][j+1],matriz[i][j]);
            cont++;
        }
    }
    ...
    cont=0;n=0;
    int _i=0;
    for (int i = 0; i < m_width*3; i=i+3){
        for (int j = 0; j < m_height; j++){
            n=(cont*k)%m_height;
            writePixel(_i,n,matriz[j][i+2],matriz[j][i+1],matriz[j][i]);
            cont++;
        }
        _i++;
    }
...
void BitMap::decrypt(char* _file, int k){
    //std::ofstream out(_file);
    //makeCopy(_file);
    ...
    for (int i = 0; i < m_height; i++){
        for (int j = 0; j < m_width*3; j=j+3){
            n=(j*k)%(m_width*3);
            //std::cout<<n<<" ";
            writePixel(cont,i,matriz[i][n+2],matriz[i][n+1],matriz[i][n]);
            cont=(cont+1)%m_width;
        }
    }
    ...
    for (int i = 0; i < m_width*3; i=i+3){
        for (int j = 0; j < m_height; j++){
            n=(j*k)%(m_height);
            writePixel(cont,j,matriz[n][i+2],matriz[n][i+1],matriz[n][i]);
        }
        cont++;
    }
